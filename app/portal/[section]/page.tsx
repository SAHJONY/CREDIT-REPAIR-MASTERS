...
const sectionCopy: Record<string, { eyebrow: string; title: string; subtitle: string }> = {
  ...
  documents: { eyebrow: 'SECURE FILES', title: 'Documents', subtitle: 'View only the files that have been shared with your portal.' },
  progress: { eyebrow: 'YOUR PLAN', title: 'Progress', subtitle: 'See where your case stands and what happens next.' },
  consents: { eyebrow: 'YOUR CHOICES', title: 'Authorizations', subtitle: 'Control the permissions you have granted for your service.' },
...
export default async function PortalSection({ params }: { params: Promise<{ section: string }> }) {
  ...
  else if (section === 'consents') content = <><section className="portalFeatureCard span6"><div className="eyebrow">YOUR AUTHORIZATIONS</div><h2>Control what we may do</h2><p>You can review and manage the permissions associated with your service.</p><PortalConsentForm /></section><section className="portalFeatureCard span6"><div className="eyebrow">HISTORY</div><h2>Recorded decisions</h2>{consents.length ? consents.map((c) => <div className="portalRecord" key={c.id}><div><strong>{c.scope.replaceAll('_',' ')}</strong><span>{new Date(c.grantedAt).toLocaleDateString()}</span></div><b className={c.granted ? 'recordGood' : 'recordBad'}>{c.granted ? 'Authorized' : 'Not authorized'}</b></div>) : <div className="portalNotice">No authorizations have been recorded yet.</div>}</section></>;
  else if (section === 'documents') content = <section className="portalFeatureCard span12"><div className="portalSectionHeading"><div><div className="eyebrow">SHARED WITH YOU</div><h2>Your secure document center</h2><p>Only files explicitly shared with your customer profile appear here.</p></div><div className="portalCount">{sharedDocuments.length}<span>documents</span></div></div>{sharedDocuments.length ? <div className="portalDocumentGrid">{sharedDocuments.map((e) => { const metadata = documentMetadata(audit, e); return <article className="portalDocumentCard" key={e.id}><div className="docIcon">DOC</div><div><strong>{e.label}</strong><span>{metadata.category.replaceAll('_',' ')}</span><small>{metadata.filename || 'document'} · {new Date(e.createdAt).toLocaleDateString()}</small></div><a className="primaryButton" href={`/api/documents/${encodeURIComponent(e.id)}/content`} target="_blank" rel="noreferrer">View / Download</a></article>; })}</div> : <div className="portalNotice">No documents have been shared with you yet. New files will appear here automatically when your account manager shares them.</div>}</section>;
  else if (section === 'progress') content = <><section className="portalFeatureCard span12"><div className="portalSectionHeading"><div><div className="eyebrow">CURRENT PLAN</div><h2>{intakeReady ? 'Your file is ready for review' : 'Complete your intake'}</h2><p>{intakeReady ? 'Your authorization and report are on file. Follow the timeline below for new activity.' : 'Your next required step is shown below.'}</p></div><div className={`portalState ${intakeReady ? 'good' : 'waiting'}`}>{intakeReady ? 'On track' : 'Action needed'}</div></div><div className="portalTimelineSteps"><div className={analysisConsent ? 'timelineDone' : ''}><i>{analysisConsent ? '✓' : '1'}</i><strong>Authorization</strong><span>{analysisConsent ? 'Complete' : 'Required'}</span></div><div className={reports.length ? 'timelineDone' : ''}><i>{reports.length ? '✓' : '2'}</i><strong>Credit report</strong><span>{reports.length ? 'Received' : 'Required'}</span></div><div className={intakeReady ? 'timelineDone' : ''}><i>{intakeReady ? '✓' : '3'}</i><strong>Review readiness</strong><span>{intakeReady ? 'Ready' : 'Waiting'}</span></div></div></section><section className="portalFeatureCard span12"><div className="eyebrow">ACTIVITY</div><h2>Your case timeline</h2>{clientAudit.length ? clientAudit.map((a) => <div className="portalRecord" key={a.id}><div><strong>{a.action.replaceAll('_',' ')}</strong><span>{new Date(a.createdAt).toLocaleString()}</span></div><b>{a.decision === 'blocked' ? 'Pending review' : a.decision === 'approval_required' ? 'Approval step' : 'Recorded'}</b></div>) : <div className="portalNotice">Your timeline will populate as work progresses.</div>}</section></>;
  else if (section === 'account') content = <><section className="portalFeatureCard span6"><div className="eyebrow">PROFILE</div><h2>{portal.client.displayName}</h2><div className="portalProfileList"><div><span>Email</span><strong>{portal.email}</strong></div><div><span>State</span><strong>{portal.client.state}</strong></div><div><span>Account status</span><strong>{portal.client.status}</strong></div></div></section><section className="portalFeatureCard span6"><div className="eyebrow">SECURITY</div><h2>Your privacy matters</h2><p>Your login is bound to one client profile. Your account cannot access staff tools or another customer's records.</p><div className="portalNotice">Never share your credit bureau passwords with CREDIT REPAIR MASTERS or anyone claiming to represent us.</div><Link className="secondaryButton" href="/portal/forgot-password">Reset portal password</Link></section></>;
  ...
  return <main className="portalShell"><header className="portalHeader"><div><div className="portalBrand">CREDIT REPAIR MASTERS</div><div className="eyebrow portalPageEyebrow">{copy.eyebrow}</div><h1>{copy.title}</h1><p className="subtitle">{copy.subtitle}</p></div><PortalNav /></header><section className="grid">{content}</section></main>;
}