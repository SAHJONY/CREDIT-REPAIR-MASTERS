  ...
  return (
    <main>
      <header className="appHeader">
        <div><div className="kicker">{organization?.name || 'CREDIT REPAIR MASTERS'} / OWNER OS / v4.0</div><h1>Operations Command Center</h1><p className="subtitle">Live tenant data from Neon. Signed in as {session.email} · {session.member.role} · MFA assured.</p></div>
        <div className="headerActions"><Link className="primaryButton" href="/launch">Launch Center</Link><Link className="secondaryButton" href="/demo">Demo OS</Link><Link className="secondaryButton" href="/billing">Billing</Link><Link className="secondaryButton" href="/clients">Clients</Link><SignOutButton /></div>
      </header>

      <section className="grid">
        <div className="card span3"><div className="label">Real clients</div><div className="value">{realClients.length}</div><div className="small">{activeClients} active · {onboarding} onboarding</div></div>
        ...
        <div className="card span7">
          ...
          {clients.length ? clients.slice(0, 8).map((client) => (
            <Link className="listRow" href={`/clients/${client.id}`} key={client.id}>
              ...
              <span className={`pill ${isDemoClient(client) ? 'medium' : 'low'}`}>{isDemoClient(client) ? 'demo' : client.status}</span>
            </Link>
        ...
        <div className="card span5">
          ...
          {!checks.some((check) => check.requiredForProduction && check.status !== 'ready') ? <div className="emptyState">All required production controls are ready.</div> : null}
        </div>
