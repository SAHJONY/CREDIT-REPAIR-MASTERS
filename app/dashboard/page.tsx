  ...
  return (
    <main>
      <header className="ownerHero">
        <div><div className="kicker">{organization?.name || 'CREDIT REPAIR MASTERS'} / APPROVAL READINESS OS / v9</div><h1>Credit-to-Approval Readiness Command Center</h1><p className="subtitle">Operate the full journey from client goal and readiness assessment through blocker reduction, evidence, Ready-to-Shop planning and compliant revenue.</p></div>
        <div className="ownerNav">{session.member.role === 'owner' ? <Link className="primaryButton" href="/owner">Private Owner OS</Link> : null}<Link className="secondaryButton" href="/loan-readiness">Approval Readiness</Link><Link className="secondaryButton" href="/clients">Clients</Link><Link className="secondaryButton" href="/billing">Billing</Link><Link className="secondaryButton" href="/documents">Documents</Link><Link className="secondaryButton" href="/growth">Growth</Link><Link className="secondaryButton" href="/launch">Launch</Link><SignOutButton /></div>
      </header>
      ...
      <section className="ownerActionStrip">
        <div className="ownerActionCard"><div className="label">OWNER FOCUS</div><h2>{openGates.length ? 'Close the final readiness-to-revenue gates' : 'Prove the Credit-to-Approval business model'}</h2><p>{openGates.length ? `${openGates.length} required control${openGates.length === 1 ? '' : 's'} still need attention before the system can consider the business fully production-ready.` : 'Required production controls are ready. Prioritize real leads, measurable client readiness improvement, compliant settlement and recurring revenue.'}</p><div className="headerActions">{session.member.role === 'owner' ? <Link className="primaryButton" href="/owner">Open Private Owner OS</Link> : <Link className="primaryButton" href="/loan-readiness">Open Approval Readiness</Link>}<Link className="secondaryButton" href={openGates.length ? '/launch' : '/growth'}>{openGates.length ? 'Open Launch Center' : 'Open Growth OS'}</Link><Link className="secondaryButton" href="/clients">Manage clients</Link></div></div>
