  ...
  return (
    <main>
      <header className="appHeader">
        <div>
          <div className="kicker">CREDIT REPAIR MASTERS / COMMERCIAL MODEL</div>
          <h1>Services & pricing</h1>
          <p className="subtitle">A diversified revenue model across consumer intelligence, business advisory, and B2B software.</p>
        </div>
      ...
      <section className="grid">
        {groups.map((group) => (
          <div className="card span12" key={group.title}>
            <div className="label">Revenue line</div>
            <h2>{group.title}</h2>
            <p className="small">{group.note}</p>
            <div className="grid" style={{ marginTop: 14 }}>
              {group.services.map((service) => (
                <div className="card span4" key={service.id}>
                  <div className="label">{service.billingModel.replaceAll('_', ' ')}</div>
                  <h2>{service.name}</h2>
                  <div className="value">{price(service)}</div>
                  <p className="small">{service.description}</p>
                  <div style={{ marginTop: 12 }}>
                    {service.deliverables.map((item) => <div className="small" key={item}>• {item}</div>)}
                  </div>
        ...
        <div className="card span12">
          <div className="label">Billing safety</div>
          <h2>No payment is collected from this page</h2>
          <p className="small">The billing eligibility API must approve the jurisdiction, sales channel, service status, contract status, and cancellation timing before a consumer-credit invoice or checkout can be enabled.</p>
        </div>
      </section>
    </main>
  );
