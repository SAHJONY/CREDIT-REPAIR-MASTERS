...
export default async function PortalSection({ params }: { params: Promise<{ section: string }> }) {
  ...
  const [consents, evidence, audit] = await Promise.all([
    ...
    store.listAudit(portal.organizationId, 100)
  ]);
  ...
  else if (section === 'documents') content = <div className="card span12"><div className="label">Private documents</div><h2>Your secure evidence ledger</h2>{evidence.length ? evidence.map((e) => <div className="listRow" key={e.id}><div><strong>{e.label}</strong><div className="small">{e.type.replaceAll('_',' ')} · {new Date(e.createdAt).toLocaleString()}</div></div><span className={`pill ${e.verification === 'verified' ? 'low' : e.verification === 'rejected' ? 'high' : 'medium'}`}>{e.verification}</span></div>) : <div className="emptyState">No documents have been added yet.</div>}</div>;
