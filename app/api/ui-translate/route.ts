...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) return NextResponse.json({ error: 'TRANSLATION_SERVICE_NOT_CONFIGURED' }, { status: 503 });
  ...
  if (!response.ok) return NextResponse.json({ error: `TRANSLATION_MODEL_FAILED_${response.status}` }, { status: 502 });
  ...
  try { parsed = JSON.parse(raw) as Record<string,string>; } catch { return NextResponse.json({ error: 'TRANSLATION_MODEL_INVALID_JSON' }, { status: 502 }); }
