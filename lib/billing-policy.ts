...
export function evaluateBillingEligibility(input: BillingEligibilityInput): BillingEligibilityResult {
  ...
  if (!service) {
    return { decision: 'blocked', mayCollectNow: false, serviceId: input.serviceId, state, reasons: ['Unknown service.'], requiredActions: ['Select a configured commercial service.'] };
  }

  if (!STATE_CODES.has(state)) {
    return { decision: 'blocked', mayCollectNow: false, serviceId: service.id, state, reasons: ['Unknown or unsupported jurisdiction.'], requiredActions: ['Resolve the client jurisdiction before billing.'] };
  }

  if (service.regulatoryClass !== 'consumer_credit_services') {
    if (service.paymentPolicy === 'manual_quote') {
      return { decision: 'manual_review', mayCollectNow: false, serviceId: service.id, state, reasons: ['Custom-priced service requires an approved quote and contract.'], requiredActions: ['Approve quote and contract before invoicing.'] };
    }
  ...
  if (!input.contractSigned) {
    requiredActions.push('Complete the required written service agreement and disclosures.');
  }

  if (!input.cancellationWindowExpired) {
    requiredActions.push('Confirm the applicable cancellation period has expired before collection.');
  }
  ...
  if (!input.contractSigned || !input.cancellationWindowExpired) {
    return { decision: 'deferred', mayCollectNow: false, serviceId: service.id, state, reasons, requiredActions };
  }
